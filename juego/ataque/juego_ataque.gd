...
func reset_state() -> void:
	...
	GameLogger.info("JuegoAtaque", "--- %s ---" % titulo_nivel)
	GameLogger.info("JuegoAtaque", "Inicio: %s, Meta: %s" % [start_node, target_node])
	if not waypoints.is_empty():
		GameLogger.info("JuegoAtaque", "Waypoints: %s" % str(waypoints))
...
func _mover_jugador(destino: StringName) -> void:
	...
	mensaje_estado = ""
	GameLogger.debug("JuegoAtaque", "[Turno %d] Jugador: %s → %s" % [turn + 1, player_pos, destino])
